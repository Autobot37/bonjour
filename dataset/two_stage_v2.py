"""
Two-Stage V2: Hierarchical Shift + Stationary Residual Learning
================================================================
Stage 1: Train a LightGBM on Day 48 to learn the baseline demand surface.
         Use 5-fold OOF predictions for Day 48 rows to prevent leakage.
         Use the full-fit model to predict Day 49 and Test base demand.

Shift:   Compute hierarchical shift ratios (D49 morning / D48 morning)
         with Bayesian shrinkage: Global -> RoadType -> G4 -> G5 -> Geohash.

Stage 2: Align the target across days by subtracting/dividing the shift-adjusted base:
         - Day 48 base: d48_oof_base (shift = 1.0)
         - Day 49 base: d49_d48_base * shift_ratio
         Residual target is stationary (mean ~ 0 on both days).
         Train LightGBM to predict this residual and add/multiply it back at test time.
"""
...
# ==================================================================
print("=" * 70)
print("TWO-STAGE V2: HIERARCHICAL SHIFT + STATIONARY RESIDUAL LEARNING")
print("=" * 70)
...
# ==================================================================
# STAGE 1: GAP-FILL D48 + OOF ESTIMATION
# ==================================================================
print("\n" + "=" * 70)
print("STAGE 1: GAP-FILL D48 (learn complete D48 surface)")
print("=" * 70)
...
print("Running 5-fold OOF predictions on D48...")
d48_oof_base = np.zeros(len(d48))
kf = KFold(n_splits=5, shuffle=True, random_state=42)
for fold, (trn_idx, val_idx) in enumerate(kf.split(d48)):
    fold_trn = d48.iloc[trn_idx]
    fold_val = d48.iloc[val_idx]
    m_fold = lgb.LGBMRegressor(**s1_params)
    m_fold.fit(fold_trn[s1_features], fold_trn['demand'])
    d48_oof_base[val_idx] = m_fold.predict(fold_val[s1_features])

print("Training final Stage 1 model on all D48...")
s1_model = lgb.LGBMRegressor(**s1_params)
s1_model.fit(d48[s1_features], d48['demand'])

# Predict base for D49 and test
d49_d48_base = s1_model.predict(d49[s1_features])
test_d48_base = s1_model.predict(test[s1_features])

# Print Stage 1 stats
print(f"  Stage 1 OOF R²: {r2_score(d48['demand'], d48_oof_base)*100:.4f}%")
print(f"  Stage 1 Self-Fit R²: {r2_score(d48['demand'], s1_model.predict(d48[s1_features]))*100:.4f}%")
...
def compute_hierarchical_shift(d48_df, d49_df, m_shrink=10):
    """
    Compute shift ratios hierarchically: Global -> RoadType -> G4 -> G5 -> Geohash
    ...
    for g4 in all_data['geohash4'].unique():
        ...
        # Find road type of this g4 (approximate using most common or global)
    ...
    # Clip shifts to reasonable bounds
    for k in gh_shifts:
...
def build_stage2_dataset(df, base_pred_vals, shifts=None, is_d48=False):
    t = df.copy()
    t['stage1_base'] = base_pred_vals
    ...
    orig_g5 = df['geohash5'].values
    hours = df['hour'].values
    
    keys_gh = list(zip(orig_gh, hours))
    t['d48_same_hour_mean'] = pd.Series([d48_geo_hour_mean.get(k, np.nan) for k in keys_gh], index=t.index).astype(float)
    t['d48_same_hour_std'] = pd.Series([d48_geo_hour_std.get(k, np.nan) for k in keys_gh], index=t.index).astype(float)
    
    if is_d48:
        # Avoid direct target leakage on Day 48
        t['d48_same_hour_mean'] = np.nan
        t['d48_same_hour_std'] = np.nan
        
    # Fill same-hour defaults
...
for m_shrink in [5, 10, 20, 50]:
    ...
    d48_s2 = build_stage2_dataset(d48, d48_oof_base, is_d48=True)
    d49_s2 = build_stage2_dataset(d49, d49_d48_base, shifts, is_d48=False)
    test_s2 = build_stage2_dataset(test, test_d48_base, shifts, is_d48=False)
    ...
    # --- Experiment C: Direct demand predicting (with base and shift as features) ---
...
# Let's check internal validation bias on best model setup
# Train on d48 + d49 h0-1, predict d49 h2
...
d49_h01_s2 = build_stage2_dataset(d49_h01, d49_h01_base, shifts_val, is_d48=False)
d49_h2_s2 = build_stage2_dataset(d49_h2, d49_h2_base, shifts_val, is_d48=False)

train_ref = pd.concat([d48, d49_h01], ignore_index=True)
d48_s2_val = build_stage2_dataset(d48, d48_oof_base, is_d48=True)
...
out = os.path.join(BASE, "dataset", "two_stage_v2_opt.csv")
