...
def main():
    ...
    if os.path.exists(real_test_path):
        ...
        print("=========================================")
        print("CATEGORY-WISE REAL LEADERBOARD SCORES")
        print("=========================================")
        cat_cols = ['Weather', 'RoadType', 'LargeVehicles', 'Landmarks']
        for col in cat_cols:
            if col in df_real.columns:
                print(f"\nFeature: {col}")
                col_series = df_real[col].fillna('Missing').astype(str)
                for val in sorted(col_series.unique()):
                    mask = col_series == val
                    y_true_cat = df_real.loc[mask, 'demand']
                    y_pred_cat = df_real.loc[mask, 'preds']
                    if len(y_true_cat) > 0:
                        cat_score = r2_score(y_true_cat, y_pred_cat) * 100
                        print(f"  {val:<15}: {cat_score:10.6f} (n={len(y_true_cat)})")
        print("=========================================\n")
